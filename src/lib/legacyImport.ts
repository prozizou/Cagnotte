...
import { createCagnotte } from "@/lib/data/cagnottes";
 ...
 * Réservé à ce seul compte : l'ancienne cagnotte n'avait pas de
 * propriétaire, ces données sont donc rattachées manuellement à
 * l'utilisateur désigné. Ce module n'est destiné qu'à un usage unique ;
 * il peut être supprimé une fois l'import effectué.
 */
...
export async function isLegacyAlreadyImported(uid: string): Promise<boolean> {
  const q = query(
...
export async function importLegacyCagnotte(actor: { uid: string; name: string }): Promise<void> {
  if (actor.uid !== LEGACY_OWNER_UID) {
    throw new Error("Import réservé au compte désigné.");
  }
  if (await isLegacyAlreadyImported(actor.uid)) {
    throw new Error("Cette cagnotte a déjà été importée.");
  }

  const cagnotteId = await createCagnotte(
    {
      title: "Éclairage & Sécurité — Quartier HLM 2",
  ...
  // Marqueur anti-doublon posé après coup (le formulaire de création ne
  // connaît pas ce champ) : on le fusionne directement via le batch de
  // cotisations pour rester atomique avec l'import.
  const batch = writeBatch(db);
  batch.set(doc(db, "cagnottes", cagnotteId), { legacyImportKey: LEGACY_IMPORT_KEY }, { merge: true });
  ...
  await logHistory({
    ...
    ownerId: actor.uid,
    cagnotteId,
    cagnotteTitle: "Éclairage & Sécurité — Quartier HLM 2",
    actorId: actor.uid,
